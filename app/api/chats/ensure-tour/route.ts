...
// POST /api/chats/ensure-tour - Create or get existing chat between agent and guide (for tours)
// Body: { agencyId, guideId } (jobId is optional and deprecated)
// This endpoint works the same as ensure-pair - chats are based on agent-guide pairs, not jobs/tours
export async function POST(req: NextRequest) {
	try {
		...
		const { data: created, error: createErr } = await supabase
			.from('chats')
			.insert({
				job_id: null, // No longer required - chats are based on agent-guide pairs
