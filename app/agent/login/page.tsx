...
function isValidAgentRedirect(path: string | null): path is string {
  if (!path || typeof path !== "string") return false;
  const decoded = decodeURIComponent(path);
  if (!decoded.startsWith("/") || decoded.startsWith("//")) return false;
  return (
    decoded.startsWith("/agent/") ||
    decoded.startsWith("/agency/") ||
    decoded === "/settings" ||
    decoded === "/profile" ||
...
function AgentLoginInner() {
  ...
  const checkingSession = useRedirectIfAuthenticated({
    redirectTo,
    isValidRedirect: isValidAgentRedirect,
  });
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const destination =
        role === "agent" && isValidAgentRedirect(redirectTo)
